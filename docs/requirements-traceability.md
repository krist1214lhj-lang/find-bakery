...
| R-003 | 부분 | 지역·빵 종류·영업·편의·검증 필터 | 탐색 필터        | BakeryLocation, SearchAlias, BreadCategory, 영업 상태 계산, VerificationRecord | Supabase 조회 저장소와 검색 변환 연결; 지역·종류·외국어 상호의 한글 별칭 검색 단위 테스트 통과 |
...
| R-014 | 부분 | 로딩·빈 결과·오류 상태           | 모든 핵심 화면   | 공통 오류 모델                                                                 | 공통 로딩·Supabase 오류·빈 결과·404 구현; 공급자별 오류 화면 미구현                            |
