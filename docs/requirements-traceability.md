...
| R-002 | 부분 | 지도/목록 전환 탐색              | 탐색             | 공통 SearchQuery, 동일 결과 ID 집합                                            | 검증 지점·미검증 후보 공통 ID, 데스크톱 분할·모바일 전환, 선택·미리보기·영역 필터 구현; 실제 SDK 키 검증 전 |
| R-003 | 부분 | 지역·빵 종류·영업·편의·검증 필터 | 탐색 필터        | BakeryLocation, SearchAlias, BreadCategory, 영업 상태 계산, VerificationRecord | 검색 조건을 지도/목록에서 유지하고 `성수동` 실제 후보 15곳 통합 표시; 영업·편의·검증 필터는 미완성 |
