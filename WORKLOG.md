...
1. **대량 수집** — ✅ 1·2·3·4라운드 완료: 1R(연남·망원·성수·서촌·한남·익선) 90곳 + 2R(합정·상수·부암동·가로수길·을지로·잠실) 85곳 + 3R(홍대·이태원·압구정·청담·강남) 61곳 + 4R(신촌·건대·노원·마포·종로) 48곳 = **`bakery_locations` 294행, 전부 라이브**. 남은 것: 미정 카테고리 다수 정밀검증(원하는 곳만)·등급(D→) 보완. **5라운드** 가능(다른 동네) — `approve-and-save.mjs --approve all --confirm`로 일괄 승인. ⚠️ stage1/2 산출물은 한 파일을 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 저장 완료 확인(현재 1~4R 모두 DB 저장 완료라 안전). 4R 신규율 75→48로 하락(서울 주요 권역 상당 커버).
