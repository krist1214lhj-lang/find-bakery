...
## 🐞 알려진 버그
- **저장된 빵집이 카카오 후보로 중복 표시** — 탐색(explore)에서 이미 DB에 저장된 빵집이 카카오 "미검증 후보"로도 함께 떠서 같은 가게가 2번 보임. 좌표/이름/주소 근접으로 후보 목록에서 기존 저장분을 걸러내야 함. (다음 과제 ②)
...
2. **중복표시 버그 수정** — 위 "알려진 버그"(저장 빵집이 카카오 후보로 중복 노출) 해결.
3. **대량 수집** — 더 많은 동네를 1·2차로 모아 작업대에서 승인·정비(현재 광진구/연남동 위주).
4. (선택) 보류(사람확인) 건 검토 흐름, 프랜차이즈 체인 ASCII slug 매핑 확대.
5. (배포 보안) 한때 노출됐던 Supabase secret key **rotate** 여부 확인 — 사용자 확인 필요
