...
- **대량수집 7라운드 완료·승인 + 숨김 재적용(2026-06-29) — 전국 광역시·관광도시 10동네 150곳 → 150곳 저장, DB 774행.** 부산 해운대·광안리·서면, 대구 동성로·수성구, 대전 둔산동, 광주 충장로, 제주 제주시·서귀포, 전주 한옥마을 각 "빵집" 15곳=150 → 2차 **승인후보 150 / 보류 0 / 제외 0**(배치내 중복 0·DB 중복 0, 전국 신규 지역). **Claude 0건 = 0원**. `--approve all --confirm`로 **150곳 저장**(건너뜀0·실패0), `bakery_locations` **624→774행**(부산 +45·대구 +30·제주 +30·대전 +15·광주 +15·전북 +15). 곧바로 `hide-uncategorized.mjs --confirm` → 신규 미분류 **139곳 draft**(백업 `hide-backup-2026-06-29T02-07-18-260Z.json`). **최종: 774행 = 공개 72(=61+7R 이름매칭 11) / draft 702 / verification_needed 1, 미분류 공개 0. 지역: 서울 482·경기 125·부산 46·대구 30·제주 30·대전 16·인천 15·광주 15·전북 15(전국 8개 시·도).**
- **대량수집 8라운드 완료·승인 + 숨김 재적용(2026-06-29) — 중소도시·관광지 10동네 150곳 → 150곳 저장, DB 924행.** 울산 삼산동·창원 상남동·청주 성안길·천안 신부동·강릉 명주동·여수 중앙동·포항 죽도시장·춘천 명동·목포·경주 황리단길 각 "빵집" 15곳=150 → 2차 **승인후보 150 / 보류 0 / 제외 0**(배치내·DB 중복 0). **Claude 0건 = 0원**. (1차에서 "목포 원도심"이 카카오 0건 → 시 단위 "목포"로 대체 후 재수집해 10/10.) `--approve all --confirm`로 **150곳 저장**(건너뜀0·실패0), `bakery_locations` **774→924행**. 곧바로 `hide-uncategorized.mjs --confirm` → 신규 미분류 **139곳 draft**(백업 `hide-backup-2026-06-29T02-15-50-242Z.json`). **최종: 924행 = 공개 83(=72+8R 이름매칭 11) / draft 841 / verification_needed 1, 미분류 공개 0. 지역 16개 시·도: 서울 482·경기 125·부산 46·강원 30·경북 30·전남 30·대구 30·제주 30·대전 16·울산 15·인천 15·충남 15·경남 15·충북 15·광주 15·전북 15(세종만 미수집).**
- **대량수집 9라운드 완료·승인 + 숨김 재적용(2026-06-29) — 세종+중소도시 10동네 136곳 저장, DB 1060행(1000 돌파)·전국 17개 시·도 완성.** 세종 어진동·안동·구미 원평동·진주 중앙시장·통영 중앙동·여수 돌산도·순천 중앙동·군산 월명동·익산 영등동·김해 내외동 → 2차 **승인후보 136 / 보류 0 / 제외 0**(배치내·DB 중복 0; 여수 돌산도 2곳도 R8 여수와 무중복). **Claude 0건 = 0원**. (1차 "안동 구시가지" 0건 → 시 단위 "안동"으로 대체; 여수 돌산도 2·군산 월명동 14로 합 136.) `--approve all --confirm`로 **136곳 저장**(건너뜀0·실패0), `bakery_locations` **924→1060행**. 곧바로 `hide-uncategorized.mjs --confirm` → 신규 미분류 **128곳 draft**(백업 `hide-backup-2026-06-29T02-29-41-478Z.json`). **최종: 1060행 = 공개 91(=83+9R 이름매칭 8) / draft 969 / verification_needed 1, 미분류 공개 0. 지역 17개 시·도(세종 추가로 전국 전부): 서울 482·경기 125·경남 60·경북 60·전남 47·부산 46·전북 44·강원 30·대구 30·제주 30·대전 16·울산 15·인천 15·세종 15·충남 15·충북 15·광주 15.**
...
- **강남·압구정·청담 검증(2026-06-29) — 같은 워크플로, A·B 8곳 공개.** `jeju-pilot.mjs --areas "강남,압구정,청담" --region2 강남구 --prefix gangnam`. **`--region2` 구 제약 신규 추가**(이유: "강남" 키워드가 도로명 "안동 강남길"까지 매칭하는 false-positive → 강남구로 한정, 47→45). draft 45(신사/가로수길·도산 22·청담 13·기타 10) → 무료 명단 → 후보 10 → 정밀검증 **717원**(A 5·B 3·보류 2) → A·B 8곳 공개. **보류 2: 디피오리(빵집 근거 없음), 타르틴베이커리 청담점(채용사이트에만 주소·공식 지점목록 없음→영업 불확실)** — 추측 금지로 미저장. 전체 공개 120→128, 강남구 공개 16, draft 1081→1073, 미분류 공개 0. 백업 `output/gangnam-save-backup-*.json`.
- **부산·대구·전주 검증(다구역 배치, 2026-06-29) — A 6곳 공개.** `jeju-pilot.mjs --districts "부산:해운대구,부산:부산진구,대구:중구,전북:전주시" --prefix busan-daegu-jeonju`. **`--districts "시도:구,…"` 다구역 선택자 신규 추가**(이유: 서면·동성로·한옥마을은 주소 필드에 없는 별칭이라 동네 키워드 매칭 불가 → 구 단위로 정확 선별; 서울 중구 을지로 vs 대구 중구 구분 위해 시도 부분일치 포함). draft 54(해운대구15·부산진구11[서면+전포동3]·대구중구13·전주시15) → 무료 명단 → 후보 9 → **사용자 지시로 PNB 3지점→대표 1곳(한옥마을점) 정리(후보 7)** → 정밀검증 **524원**(A 6·보류 1) → A 6곳 공개(옵스 해운대·마린시티·롯데부산·비엔씨 해운대·대구근대골목단팥빵·PNB풍년제과 한옥마을점). 퍼프베이커리 보류(근거 불충분). 전체 공개 128→134, draft 1073→1067, 미분류 공개 0. 백업 `output/busan-daegu-jeonju-save-backup-*.json`.
...
| `37216e5` | 대량수집 6라운드(수도권 10동네 140곳, DB 624행) + 숨김 재적용(128) + 일지 | ✅ (codex) |
| `d3ff1d6` | 대량수집 7라운드(전국 10동네 150곳, DB 774행) + 숨김 재적용(139) + 일지 | ✅ (codex) |
| `8bf91e9` | 대량수집 8라운드(중소도시 10동네 150곳, DB 924행) + 숨김 재적용(139) + 일지 | ✅ (codex) |
...
1. **대량 수집** — ✅ 1~11라운드 완료: 서울(1~5R)+수도권(6R)+전국 광역시(7R)+중소도시(8R)+세종·중소(9R)+5도시(10R)+제주 관광지(11R) = **`bakery_locations` 1201행**(공개 98/숨김 1103; **전국 17개 시·도 전부 커버, 제주 96곳**). 전국망 완성 → 다음 우선순위는 **수집보다 미정 카테고리 정밀검증으로 채워 재노출·등급(D→) 보완**. 추가 수집 원하면 12라운드(미커버 소도시) 가능 — `approve-and-save.mjs --approve all --confirm` 후 `hide-uncategorized.mjs --confirm`. ⚠️ stage1/2 산출물은 한 파일을 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 저장 완료 확인(현재 1~11R 모두 저장 완료라 안전). 소도시는 세부 동(예: "목포 원도심"·"안동 구시가지" 0건)보다 시 단위 검색이 결과 안정적.
...
| `jeju-pilot.mjs` | 지역 검증 도구(목록/`--verify`/`--save`), 선택자 `--areas`(동)/`--region2`(단일구)/`--districts`(시도:구 다구역). `verification-research` 로직 포팅 | 완성·검증공개 누계 36곳·커밋 대기 |
...
6. **대량수집 7라운드(전국 광역시·관광도시 10개) 실행 + 숨김 재적용 — 사용자 승인 후 끝까지 자동.**
   - 동네(10): 부산 해운대·광안리·서면, 대구 동성로·수성구, 대전 둔산동, 광주 충장로, 제주 제주시·서귀포, 전주 한옥마을. 각 카카오 "빵집"(공백 포함 검색어 정상) 15곳 = 합 150곳.
   - **1차(카카오)**: 10쿼리 전부 통과, 150곳 수집. 무료.
   - **2차(Claude 검증)**: 배치내 중복 0·DB 중복 0(전국 신규 지역, 기존 대전1·부산1 시드와도 미충돌) → **승인후보 150 / 보류 0 / 제외 0**. 전부 제과·베이커리+규칙 확정 → **Claude 0건 = 0원**.
   - **3차(저장)**: 드라이런 150/0 → `--approve all --confirm` → **150곳 저장**(건너뜀0·실패0). `bakery_locations` **624 → 774행**. 제주(위도~33.2)도 DB 좌표 제약(위도33~39) 내 정상.
   - **4차(숨김 재적용)**: `hide-uncategorized.mjs` 드라이런(숨김 대상 139=신규 미분류) → `--confirm` → **139건 draft(실패0)**, 백업 `output/hide-backup-2026-06-29T02-07-18-260Z.json`.
   - **최종 검증**: 총 774 / status active71·verification_needed1·draft702 / **공개(RLS) 72 전부 카테고리 보유·미분류 공개 0**(공개 72=이전 61+7R 이름매칭 11). 지역 서울 482·경기 125·부산 46·대구 30·제주 30·대전 16·인천 15·광주 15·전북 15 → **전국 8개 시·도 커버**. DB 구조 변경 0, `quest_` 미사용, 좌표·주소 카카오값만.
7. **대량수집 8라운드(중소도시·관광지 10개) 실행 + 숨김 재적용 — 사용자 승인 후 끝까지 자동.**
   - 동네(10): 울산 삼산동·창원 상남동·청주 성안길·천안 신부동·강릉 명주동·여수 중앙동·포항 죽도시장·춘천 명동·목포·경주 황리단길. 각 카카오 "빵집" 15곳.
   - **1차(카카오)**: 첫 시도에서 "목포 원도심"이 **0건**(카카오가 "원도심" 토큰 미인식) → 9/10. 목포를 빠뜨리지 않게 시 단위 "목포"로 바꿔 전체 재수집 → **10/10, 150곳**. (소도시는 세부 동보다 시 단위가 안정적 — 다음 라운드 참고.)
   - **2차(Claude 검증)**: 배치내 중복 0·DB 중복 0(전국 신규 지역) → **승인후보 150 / 보류 0 / 제외 0**. **Claude 0건 = 0원**.
   - **3차(저장)**: 드라이런 150/0 → `--approve all --confirm` → **150곳 저장**(건너뜀0·실패0). `bakery_locations` **774 → 924행**.
   - **4차(숨김 재적용)**: `hide-uncategorized.mjs` 드라이런(숨김 대상 139=신규 미분류) → `--confirm` → **139건 draft(실패0)**, 백업 `output/hide-backup-2026-06-29T02-15-50-242Z.json`.
   - **최종 검증**: 총 924 / status active82·verification_needed1·draft841 / **공개(RLS) 83 전부 카테고리 보유·미분류 공개 0**(공개 83=이전 72+8R 이름매칭 11). **지역 16개 시·도**(울산·경남·충북·충남·강원·전남·경북 신규 추가, 세종만 미수집). DB 구조 변경 0, `quest_` 미사용, 좌표·주소 카카오값만.
8. **대량수집 9라운드(세종+중소도시 10개) 실행 + 숨김 재적용 — 사용자 승인 후 끝까지 자동. DB 1000행 돌파·전국 17개 시·도 완성.**
   - 동네(10): 세종 어진동·안동·구미 원평동·진주 중앙시장·통영 중앙동·여수 돌산도·순천 중앙동·군산 월명동·익산 영등동·김해 내외동.
   - **1차(카카오)**: "안동 구시가지" **0건** → 시 단위 "안동"으로 대체 재수집(8R 목포와 동일 처리). 여수 돌산도 2건·군산 월명동 14건(15 미만)으로 합 **136곳**, 10/10.
   - **2차(Claude 검증)**: 배치내 중복 0·DB 중복 0(여수 돌산도 2곳도 R8 여수 중앙동과 무중복) → **승인후보 136 / 보류 0 / 제외 0**. **Claude 0건 = 0원**.
   - **3차(저장)**: 드라이런 136/0 → `--approve all --confirm` → **136곳 저장**(건너뜀0·실패0). `bakery_locations` **924 → 1060행**.
   - **4차(숨김 재적용)**: `hide-uncategorized.mjs` 드라이런(숨김 대상 128=신규 미분류) → `--confirm` → **128건 draft(실패0)**, 백업 `output/hide-backup-2026-06-29T02-29-41-478Z.json`.
   - **최종 검증**: 총 1060 / status active90·verification_needed1·draft969 / **공개(RLS) 91 전부 카테고리 보유·미분류 공개 0**(공개 91=이전 83+9R 이름매칭 8). **지역 17개 시·도 — 세종 추가로 전국 전부 커버 완성.** DB 구조 변경 0, `quest_` 미사용, 좌표·주소 카카오값만.
9. **대량수집 10라운드(5개 도시) 실행 + 숨김 재적용 — 사용자 승인 후 끝까지 자동.**
   - 동네(5): 원주·거제·양산·경산·아산(전부 시 단위). 각 카카오 "빵집" 15곳 = 합 75곳, 1차 5/5(0건 없음).
   - **2차(Claude 검증)**: 배치내 중복 0·DB 중복 0 → **승인후보 75 / 보류 0 / 제외 0**. **Claude 0건 = 0원**.
   - **3차(저장)**: 드라이런 75/0 → `--approve all --confirm` → **75곳 저장**(건너뜀0·실패0). `bakery_locations` **1060 → 1135행**(경남 +30·경북 +15·강원 +15·충남 +15).
   - **4차(숨김 재적용)**: `hide-uncategorized.mjs` 드라이런(숨김 대상 69=신규 미분류) → `--confirm` → **69건 draft(실패0)**, 백업 `output/hide-backup-2026-06-29T02-44-28-705Z.json`.
   - **최종 검증**: 총 1135 / status active96·verification_needed1·draft1038 / **공개(RLS) 97 전부 카테고리 보유·미분류 공개 0**(공개 97=이전 91+10R 이름매칭 6). 지역 17개 시·도 유지. DB 구조 변경 0, `quest_` 미사용, 좌표·주소 카카오값만.
10. **대량수집 11라운드(제주 관광지 5개) 실행 + 숨김 재적용 — 사용자 승인 후 끝까지 자동.**
   - 동네(5): 제주 애월·협재·성산, 서귀포 중문·대정. 각 카카오 "빵집" 15곳(성산 14) = 합 74곳, 1차 5/5.
   - **2차(Claude 검증)**: 배치내 중복 0 → 74 고유 → **승인후보 66 / 보류 0 / 제외 8**. 제외 8=R7 "제주 제주시"·"제주 서귀포" 기수집분과 약 0m 중복(예: 이층빵집굽는아방). **Claude 0건 = 0원**.
   - **3차(저장)**: 드라이런 66/0 → `--approve all --confirm` → **66곳 저장**(건너뜀0·실패0). `bakery_locations` **1135 → 1201행**(제주 30→96).
   - **4차(숨김 재적용)**: `hide-uncategorized.mjs` 드라이런(숨김 대상 65=신규 미분류) → `--confirm` → **65건 draft(실패0)**, 백업 `output/hide-backup-2026-06-29T02-53-50-906Z.json`.
   - **최종 검증**: 총 1201 / status active97·verification_needed1·draft1103 / **공개(RLS) 98 전부 카테고리 보유·미분류 공개 0**(공개 98=이전 97+11R 이름매칭 1). 제주 96곳으로 강화. DB 구조 변경 0, `quest_` 미사용, 좌표·주소 카카오값만.
11. **제주 빵집 검증 파일럿 — 숨김분 중 유명 빵집만 검증해 공개 전환 (단계별 승인).** 목표: 제주 draft 중 "진짜 유명한 곳"만 A·B등급+카테고리로 공개.
   - **1단계(무료·읽기)**: `jeju-pilot.mjs` → 제주 draft **94곳** 추출(`output/jeju-draft.json`).
   - **2단계(무료)**: WebSearch로 "제주 유명 빵집/추천/빵지순례/생활의 달인" 등 복수 검색 → 반복 등장 이름 명단(특정 사이트 복사 금지, 스니펫 종합).
   - **3단계(무료·로컬)**: 94곳 ↔ 명단 이름 매칭 → **후보 11곳**(10 + 보래드베이커스). 프랜차이즈·호텔·미언급은 제외.
...
13. **강남·압구정·청담 검증 — 같은 워크플로, A·B 8곳 공개 + `--region2` 보정.**
   - `jeju-pilot.mjs --areas "강남,압구정,청담" --region2 강남구 --prefix gangnam`.
   - **1단계**: 첫 추출 47곳에 **안동 빵집 2곳**(경북 안동 "강남1·2길" 도로명이 "강남" 키워드에 매칭) 혼입 발견 → **`--region2` 구 제약 신규 추가**(matchArea에 `region_level_2===값` 게이트) → 강남구 한정 **45곳**(신사/가로수길·도산 22·청담 13·역삼/논현/도곡/삼성 10).
   - **2~3단계(무료)**: 압구정·청담·도산·가로수길 WebSearch 명단 → 후보 **10곳**(호프만은 검색이 바르셀로나 동명점으로 떠 제외).
   - **4단계(유료)**: `--verify --prefix gangnam` → **A 5·B 3·보류 2**, 실측 **717원**.
   - **5단계(저장)**: `--save --confirm --prefix gangnam` → A·B **8곳** 공개. **보류 2(디피오리·타르틴 청담점) 미저장** — 디피오리는 빵집 언급 출처 0, 타르틴 청담점은 채용사이트에만 주소·공식 지점목록 미포함(영업 불확실)으로 동일가게 확신 불가 → 추측 금지 작동.
   - **결과**: 전체 공개 120→**128**, 강남구 공개 16, draft 1081→1073, 미분류 공개 0. 실패 0. 백업 `output/gangnam-save-backup-*.json`.
14. **부산·대구·전주 검증(다구역 배치) — A 6곳 공개 + `--districts` 신규.**
   - `jeju-pilot.mjs --districts "부산:해운대구,부산:부산진구,대구:중구,전북:전주시" --prefix busan-daegu-jeonju`.
   - **1단계**: 별칭(서면·동성로·한옥마을)이 주소에 없어 동네 키워드 매칭 불가 진단 → **`--districts "시도:구,…"` 다구역 선택자 추가**(matchArea: 시도 부분일치 AND 구 정확일치; 서울 중구 을지로와 대구 중구 구분). draft **54곳**(해운대구15·부산진구11[전포동3 포함]·대구중구13·전주시15). "부산 전포"는 별도 수집 없었으나 부산진구 전포동 3곳이 포함됨.
   - **2~3단계(무료)**: 부산 해운대/서면·전포, 대구 동성로, 전주 한옥마을 WebSearch → 후보 9곳 → **사용자 지시로 PNB 3지점→대표 1곳(한옥마을점) 정리 → 후보 7곳**.
