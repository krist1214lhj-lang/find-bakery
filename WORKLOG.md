...
| (이번) | 2차 검증 **25건 분할호출 + 배치내 placeId 중복제거** 보강 + 대량수집 1라운드 일지 | ❌ (로컬, push 미정) |
...
- **대량수집 1라운드(6동네 90곳) 완료 — 작업대 승인 대기 중.** `output/stage2-verified.json`에 90곳(전부 승인후보) 적재. 로컬 dev에서 `/admin/workbench` 열어 승인·카테고리·정밀검증 등급 부여하면 DB 반영. (수집 자체는 DB 무변경.)
...
1. **대량 수집** — ✅ 1라운드(6동네: 연남·망원·성수·서촌·한남·익선, 90곳) 완료(2026-06-27). **작업대에서 승인·카테고리·정밀검증 남음.** 더 모으려면 2라운드(합정·상수·부암동·가로수길·을지로·잠실 등)로 같은 방식. ⚠️ 작업대는 `stage2-verified.json` 한 파일만 읽어 덮어쓰므로, **1라운드 작업대 승인 후 2라운드** 진행 권장(안 그러면 1라운드 미승인분이 덮여 사라짐).
