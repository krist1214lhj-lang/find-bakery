...
| (이번) | 대량수집 5라운드(15동네 190곳 저장, DB 484행) + 숨김 재적용(172) + 일지 | ❌ (로컬, push 예정) |
...
- **대량수집 1~5라운드 승인 완료 — `bakery_locations` 484행.** 1R 90 + 2R 85 + 3R 61 + 4R 48 + 5R 190(`--approve all --confirm`). **카테고리 미연결분은 `draft`로 숨김 → 현재 공개는 49곳(카테고리 보유)만, draft 435.** 숨긴 곳은 카테고리 채우면 재노출 가능. ⚠️ 수집 라운드마다 저장(active) 직후 `hide-uncategorized.mjs --confirm`로 미분류 숨김 재적용 필요. 등급 미검증(D) — 원하는 곳만 정밀검증 예정.
...
1. **대량 수집** — ✅ 1~5라운드 완료: 1R(연남·망원·성수·서촌·한남·익선)90 + 2R(합정·상수·부암동·가로수길·을지로·잠실)85 + 3R(홍대·이태원·압구정·청담·강남)61 + 4R(신촌·건대·노원·마포·종로)48 + 5R(용산·동대문·혜화·뚝섬·왕십리·신림·영등포·여의도·목동·상암·노량진·낙성대·사당·방배·잠실새내)190 = **`bakery_locations` 484행**(공개 49/숨김 435). 남은 것: 미정 카테고리 정밀검증으로 채워 재노출·등급(D→) 보완. **6라운드** 가능(미커버 지역) — `approve-and-save.mjs --approve all --confirm` 후 `hide-uncategorized.mjs --confirm`. ⚠️ stage1/2 산출물은 한 파일을 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 저장 완료 확인(현재 1~5R 모두 저장 완료라 안전).
