...
| (이번) | `jeju-pilot.mjs` `--region2` 추가 + 강남 A·B 8곳 공개(공개 128) + 일지 | ❌ (로컬, push 예정) |
...
| `jeju-pilot.mjs` | 지역 검증 파일럿(목록/`--verify`/`--save`), **지역 파라미터화**(`--areas`/`--region2`/`--prefix`, 미지정=제주). `verification-research` 로직 포팅 | 완성·제주10+서울12+강남8 공개·커밋 대기 |
