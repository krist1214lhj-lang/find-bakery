...
| (이번) | 대량수집 7라운드(전국 10동네 150곳, DB 774행) + 숨김 재적용(139) + 일지 | ❌ (로컬, push 예정) |
...
- **대량수집 1~7라운드 승인 완료 — `bakery_locations` 774행**(서울 482·경기 125·부산 46·대구 30·제주 30·대전 16·인천 15·광주 15·전북 15, 전국 8개 시·도). 1R 90 + 2R 85 + 3R 61 + 4R 48 + 5R 190 + 6R(수도권) 140 + 7R(전국) 150(`--approve all --confirm`). **카테고리 미연결분은 `draft`로 숨김 → 현재 공개는 72곳(카테고리 보유)만, draft 702.** 숨긴 곳은 카테고리 채우면 재노출 가능. ⚠️ 수집 라운드마다 저장(active) 직후 `hide-uncategorized.mjs --confirm`로 미분류 숨김 재적용 필요. 등급 미검증(D) — 원하는 곳만 정밀검증 예정.
...
1. **대량 수집** — ✅ 1~7라운드 완료: 1R 90 + 2R 85 + 3R 61 + 4R 48 + 5R 190(서울) + 6R(수도권)140 + 7R(전국: 부산·대구·대전·광주·제주·전주)150 = **`bakery_locations` 774행**(공개 72/숨김 702; 전국 8개 시·도). 남은 것: 미정 카테고리 정밀검증으로 채워 재노출·등급(D→) 보완. **8라운드** 가능(미커버 지역) — `approve-and-save.mjs --approve all --confirm` 후 `hide-uncategorized.mjs --confirm`. ⚠️ stage1/2 산출물은 한 파일을 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 저장 완료 확인(현재 1~7R 모두 저장 완료라 안전).
