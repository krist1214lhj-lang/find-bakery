...
| (이번) | 대량수집 9라운드(세종+중소 10동네 136곳, DB 1060행·17개시도완성) + 숨김(128) + 일지 | ❌ (로컬, push 예정) |
...
- **대량수집 1~9라운드 승인 완료 — `bakery_locations` 1060행**(전국 17개 시·도 전부: 서울 482·경기 125·경남 60·경북 60·전남 47·부산 46·전북 44·강원 30·대구 30·제주 30·대전 16·울산 15·인천 15·세종 15·충남 15·충북 15·광주 15). 1R 90 + 2R 85 + 3R 61 + 4R 48 + 5R 190 + 6R(수도권) 140 + 7R(전국) 150 + 8R(중소도시) 150 + 9R(세종·중소) 136(`--approve all --confirm`). **카테고리 미연결분은 `draft`로 숨김 → 현재 공개는 91곳(카테고리 보유)만, draft 969.** 숨긴 곳은 카테고리 채우면 재노출 가능. ⚠️ 수집 라운드마다 저장(active) 직후 `hide-uncategorized.mjs --confirm`로 미분류 숨김 재적용 필요. 등급 미검증(D) — 원하는 곳만 정밀검증 예정.
...
1. **대량 수집** — ✅ 1~9라운드 완료: 서울(1~5R)+수도권(6R)+전국 광역시(7R)+중소도시(8R)+세종·중소(9R) = **`bakery_locations` 1060행**(공개 91/숨김 969; **전국 17개 시·도 전부 커버 완성**). 전국망 사실상 완성 → 다음 우선순위는 **수집보다 미정 카테고리 정밀검증으로 채워 재노출·등급(D→) 보완**. 추가 수집 원하면 10라운드(미커버 소도시) 가능 — `approve-and-save.mjs --approve all --confirm` 후 `hide-uncategorized.mjs --confirm`. ⚠️ stage1/2 산출물은 한 파일을 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 저장 완료 확인(현재 1~9R 모두 저장 완료라 안전). 소도시는 세부 동(예: "목포 원도심"·"안동 구시가지" 0건)보다 시 단위 검색이 결과 안정적.
