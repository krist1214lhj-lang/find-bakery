...
1. **파이프라인 마지막 두 조각: 내 승인 단계 → DB 저장 단계** 설계·구현. 저장은 `supabase-admin` 방식으로 쓰기(승인 후).
2. **보류(사람확인) 건을 검토하는 관리자 페이지** — `output/stage2-verified.json`의 보류 목록을 보고 승인/제외를 결정하는 화면.
3. push (승인 후): `03928c1`(1차), `10aa4c1`(문서), `ea6aa3d`(2차 옛판), `dd2476c`(2차 재작성).
