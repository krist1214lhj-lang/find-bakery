...
| (이번) | 3단계 `approve-and-save` 추가 + 일지 갱신 | ❌ (push 대기) |

---

## 🔄 진행 중

- **관리자 페이지** 설계 — 검증·등급 부여 / 카테고리 보완 / 보류 건 검토 ← 바로 다음 작업
...
1. **관리자 페이지**: (a) 자동수집 빵집 **검증·신뢰등급(A~D) 부여**(현재 미검증/D수준), (b) **카테고리 미정** 빵집 보완, (c) 2차 **보류(사람확인)** 건 검토(승인/제외).
...
| `approve-and-save.mjs` | 3차: 승인 후 DB 저장(브랜드·위치·카테고리), 플래그 3단계·idempotent | 완성·첫 저장 검증·커밋 대기 |
