...
| (이번) | 대량수집 8라운드(중소도시 10동네 150곳, DB 924행) + 숨김 재적용(139) + 일지 | ❌ (로컬, push 예정) |
...
- **대량수집 1~8라운드 승인 완료 — `bakery_locations` 924행**(전국 16개 시·도: 서울 482·경기 125·부산 46·강원 30·경북 30·전남 30·대구 30·제주 30·대전 16·울산 15·인천 15·충남 15·경남 15·충북 15·광주 15·전북 15, 세종만 미수집). 1R 90 + 2R 85 + 3R 61 + 4R 48 + 5R 190 + 6R(수도권) 140 + 7R(전국) 150 + 8R(중소도시) 150(`--approve all --confirm`). **카테고리 미연결분은 `draft`로 숨김 → 현재 공개는 83곳(카테고리 보유)만, draft 841.** 숨긴 곳은 카테고리 채우면 재노출 가능. ⚠️ 수집 라운드마다 저장(active) 직후 `hide-uncategorized.mjs --confirm`로 미분류 숨김 재적용 필요. 등급 미검증(D) — 원하는 곳만 정밀검증 예정.
...
1. **대량 수집** — ✅ 1~8라운드 완료: 서울(1~5R)+수도권(6R)+전국 광역시(7R)+중소도시(8R) = **`bakery_locations` 924행**(공개 83/숨김 841; 전국 16개 시·도, 세종만 미수집). 남은 것: 미정 카테고리 정밀검증으로 채워 재노출·등급(D→) 보완. **9라운드**(세종·미커버 소도시) 가능 — `approve-and-save.mjs --approve all --confirm` 후 `hide-uncategorized.mjs --confirm`. ⚠️ stage1/2 산출물은 한 파일을 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 저장 완료 확인(현재 1~8R 모두 저장 완료라 안전). 소도시는 세부 동(예: "목포 원도심")보다 시 단위 검색이 결과 안정적.
