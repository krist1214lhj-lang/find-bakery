...
## 바로 다음 할 일
1. **미정 카테고리·등급 보완** — 1~4라운드 수집분 **284곳이 라이브지만 다수 카테고리 미정·등급 D**(이름매칭만 일부 연결). 작업대 `/admin/workbench`에서 원하는 곳만 **정밀 검증(웹검색)**으로 등급+카테고리(곳당 60~75원).
...
- **DB 상태**: `bakery_locations` **294행**(시드 10 + 1라운드 90 + 2라운드 85 + 3라운드 61 + 4라운드 48, 전부 승인·라이브). 수집분 284곳 미검증 D·카테고리 다수 미정. 재실행해도 중복 안 들어감(멱등).
