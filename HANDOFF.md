...
## 바로 다음 할 일
1. **미정 카테고리 채워 재노출** — 카테고리 미연결 **263곳은 `draft`로 숨김** 상태(공개 31곳만). 작업대 `/admin/workbench`에서 **정밀 검증(웹검색)**으로 카테고리+등급 부여(곳당 60~75원) → 카테고리 생기면 `status='active'`로 되돌려 재노출(published_at 보존됨). 대량 재노출은 `hide-uncategorized.mjs --restore`(백업 원복) 또는 카테고리 부여 후 재활성 스크립트.
...
- **DB 상태**: `bakery_locations` **294행**, 그중 **공개 31곳(카테고리 보유)** / **숨김 263곳(`status='draft'`, 카테고리 미연결, published_at 보존)**. 공개 노출 = RLS `status<>'draft' AND published_at IS NOT NULL`(코드 status 필터 없음). 카테고리 채우면 작업대/스크립트로 다시 active 가능. 재실행해도 중복 안 들어감(멱등).
