...
## 바로 다음 할 일
1. **미정 카테고리 채워 재노출 (진행 중)** — 카테고리 미연결 **1093곳이 `draft`로 숨김**(공개 108곳). **제주 파일럿으로 표준 워크플로 확립**: 무료 WebSearch 명단 → 후보만 유료검증(곳당 ~70원) → A·B만 공개. `scripts/jeju-pilot.mjs` 참고. 다른 지역(부산·서울 등)에 같은 방식 확장하면 됨. 보룡제과 등 보류건은 Sonnet 재검증/수동 등급 가능. 작업대 `/admin/workbench`에서 **정밀 검증(웹검색)**으로 카테고리+등급 부여(곳당 60~75원) → 카테고리 생기면 `status='active'`로 되돌려 재노출(published_at 보존됨). 대량 재노출은 `hide-uncategorized.mjs --restore`(백업 원복) 또는 카테고리 부여 후 재활성 스크립트.
...
- **DB 상태**: `bakery_locations` **1201행**(10라운드 1135 + 11R 제주 66), 그중 **공개 108곳** / **숨김 1093곳(`status='draft'`)** / verification_needed 1. 공개 108 = 이름매칭 카테고리분 + **제주 파일럿 검증 10곳(A·B)**. 공개분 전부 카테고리 보유, 미분류 공개 0. 지역 **17개 시·도 전부**: 서울 482 · 경기 125 · 제주 96 · 경남 90 · 경북 75 · 전남 47 · 부산 46 · 강원 45 · 전북 44 · 충남 30 · 대구 30 · 대전 16 · 울산 15 · 인천 15 · 세종 15 · 충북 15 · 광주 15. 공개 노출 = RLS `status<>'draft' AND published_at IS NOT NULL`(코드 status 필터 없음). 카테고리 채우면 작업대/스크립트로 다시 active 가능. 재실행해도 중복 안 들어감(멱등). ⚠️ approve-and-save는 신규를 active로 저장 → 수집 라운드 뒤엔 `hide-uncategorized.mjs --confirm`로 미분류 숨김 재적용 필요.
