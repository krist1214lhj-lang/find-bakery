...
- **카테고리 이름 자동매칭(무료) 규칙 제시 — 실행 대기.** 추측 금지 원칙으로 **이름에 단서가 명확할 때만** 6종 카테고리 매칭(`approve-and-save.mjs`의 `CAT_KEYWORDS`와 동일): 베이글←베이글 / 크루아상←크루아상·크로와상·크라상·크루아쌍 / 소금빵←소금빵 / 케이크←케이크·케익 / 구움과자←쿠키·구움과자·휘낭시에·마들렌·스콘 / 식사빵←식사빵·바게트·치아바타·캄파뉴. 단서 없으면 미정(추측 안 함). 작업대 승인 시 자동 미리체크되며, 미정은 정밀검증(웹검색)으로 보완. *(사용자 OK 후 90곳 매칭 리포트 산출 예정.)*
- (참고) `bakery_locations` 현재 **11행**(시드 포함; 90곳 대량수집분은 미저장).
...
## 바로 다음 할 일
1. **작업대에서 90곳 승인·정비** — `output/stage2-verified.json`(1라운드 90곳)을 로컬 `/admin/workbench`에서 승인 → 카테고리 → 정밀 검증(등급). 카테고리는 이름 자동매칭이 미리 체크됨, 미정은 원하는 곳만 정밀검증(곳당 60~75원).
2. **(대기) 카테고리 이름 자동매칭 리포트** — 사용자 OK 시 90곳에 `CAT_KEYWORDS` 적용해 "매칭 N곳 / 미정 N곳" 보고(무료, DB 무변경).
3. **대량 수집 2라운드** — 나머지 동네(합정·상수·부암동·가로수길·을지로·잠실 등) 같은 방식. ⚠️ 작업대는 `stage2-verified.json` 한 파일만 읽어 덮어쓰므로 **1라운드 승인 후** 진행(안 그러면 미승인분 덮임).
- (선택) 보류 건 검토 흐름, 프랜차이즈 체인 ASCII slug 매핑 확대. 정밀 검증 정확도 아쉬우면 `VERIFY_MODEL`→Sonnet.
- ~~검증등급 자동화~~/~~중복표시 버그 수정~~ → **2026-06-26 완료**. ~~대량수집 1라운드~~ → **2026-06-27 완료**.
...
- **DB 상태**: `bakery_locations` 11행(시드 포함). 스택베이커리(slug=`kakao-78696240`) 등 active. 재실행해도 중복 안 들어감.
- **대량수집 1라운드 산출물**: `output/stage2-verified.json`에 90곳(전부 승인후보) 대기 — **작업대 승인 전(DB 미저장).** 로컬 커밋 `03ef826` **push 대기**. 2라운드는 승인 후(파일 덮어쓰기 주의).
