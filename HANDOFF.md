...
**2026-06-29**
- **대량수집 3라운드 완료·승인 — 5동네 75곳 수집 → 61곳 라이브 저장.** 홍대·이태원·압구정·청담·강남 각 카카오 "빵집" 15곳=75 → 배치내 중복 6 제거(69 고유) → 2차 **승인후보 61 / 보류 0 / 제외 8**(기존 DB 중복 약 0m: 홍대·상수권 7곳 + 가로수길 인근 외계인방앗간 등). 전부 카카오 `제과,베이커리` + 규칙 확정 → **Claude 호출 0건 = 2차 비용 0원**. `approve-and-save.mjs --approve all --confirm`로 61곳 일괄 저장(건너뜀 0·실패 0, 이름매칭 카테고리 자동연결 예: 바게트케이→식사빵), `bakery_locations` **185→246행**. 프랜차이즈(파리바게뜨 청담사거리점 등) 표시만. 미검증 D·카테고리 다수 미정 → 정밀검증 추후.
- **대량수집 4라운드 완료·승인 — 5동네 75곳 수집 → 48곳 라이브 저장.** 신촌·건대·노원·마포·종로 각 카카오 "빵집" 15곳=75 → 2차 **승인후보 49 / 보류 0 / 제외 26**(예상대로 마포=합정·상수·망원·연남, 종로=서촌·익선·을지로 등 기존 라운드와 광역 중복 자동제외). 전부 규칙 확정 → **Claude 호출 0건 = 2차 비용 0원**. `--approve all --confirm`로 저장 시 1곳(나폴레옹과자점 롯데백화점 건대점)이 저장 직전 재조회 중복으로 건너뜀 → **48곳 저장**(실패 0). `bakery_locations` **246→294행**. 프랜차이즈 표시만. 미검증 D·카테고리 다수 미정 → 정밀검증 추후.
- **카테고리 미연결 빵집 일괄 숨김(`status='draft'`) — 공개 294 → 31곳.** `location_bread_categories` 연결이 0개인 **263곳**을 `active→draft`로 변경(카테고리 ≥1인 **31곳만 공개 유지**). ⚠️ **`'hidden'`은 `location_status` enum에 없음**(menu 전용) → 공개 노출은 RLS **`status<>'draft' AND published_at IS NOT NULL`**가 결정(공개 목록 쿼리는 status 필터 없이 anon RLS 의존, `lib/bakery-repository.ts:146`). 그래서 비공개 수단 = `status='draft'`(published_at 보존 → 되돌리기는 status만 active). 새 스크립트 `scripts/hide-uncategorized.mjs`(드라이런/`--confirm`/`--restore`). 변경 직전 백업 `output/hide-backup-*.json`(gitignored). 검증: 공개 31 전부 카테고리 보유·미연결 공개 0. 멱등.
- **대량수집 5라운드 완료·승인 + 숨김 재적용 — 15동네 225곳 수집 → 190곳 저장, 공개는 카테고리 보유분만.** 용산·동대문·혜화·뚝섬·왕십리·신림·영등포·여의도·목동·상암·노량진·낙성대·사당·방배·잠실새내 각 "빵집" 15곳=225 → 배치내 중복 6 제거(219 고유) → 2차 **승인후보 190 / 보류 2 / 제외 27**(제외=뚝섬·잠실새내 등 기존 중복, 보류 2=하네단 27m·빵에무아 21m 근접 가게 사람확인 대기). **Claude 검토 2건(≈1.5원)**. `--approve all --confirm`로 **190곳 저장**(실패 0), `bakery_locations` **294→484행**. approve-and-save는 신규를 active로 저장하므로 정책 유지 위해 곧바로 `hide-uncategorized.mjs --confirm` 재실행 → 신규 미분류 **172곳 draft**(백업 `hide-backup-2026-06-29T01-43-46-775Z.json`). **최종: 484행 중 공개 49(카테고리 보유, 31+5R 이름매칭 18) / draft 435 / verification_needed 1**, 미분류 공개 0. 신규율 회복(서남부 신규 동네 다수).
...
## 바로 다음 할 일
1. **미정 카테고리 채워 재노출** — 카테고리 미연결 **435곳이 `draft`로 숨김** 상태(공개 49곳만). 작업대 `/admin/workbench`에서 **정밀 검증(웹검색)**으로 카테고리+등급 부여(곳당 60~75원) → 카테고리 생기면 `status='active'`로 되돌려 재노출(published_at 보존됨). 대량 재노출은 `hide-uncategorized.mjs --restore`(백업 원복) 또는 카테고리 부여 후 재활성 스크립트.
2. **대량 수집 5라운드(선택)** — 또 다른 동네. 수집(stage1→2) 후 `approve-and-save.mjs --approve all --confirm`로 일괄 승인. ⚠️ 스크립트는 `stage1/2` 산출물 한 파일만 읽어 덮어쓰므로 다음 라운드 stage2 전 직전 라운드 DB 저장 완료 확인(현재 1~4R 모두 저장 완료라 안전). (참고: 서울 주요 빵집 권역은 1~4R로 상당 부분 커버 — 신규율 점차 하락, 4R은 75곳 중 48곳만 신규.)
- (선택) 보류 건 검토 흐름, 프랜차이즈 체인 ASCII slug 매핑 확대. 정밀 검증 정확도 아쉬우면 `VERIFY_MODEL`→Sonnet.
- ~~검증등급 자동화~~/~~중복표시 버그 수정~~ → **2026-06-26 완료**. ~~대량수집 1라운드(수집·승인)~~/~~카테고리 자동 미리체크~~/~~탐색 단일핀(PR #4)~~/~~대량수집 2라운드(85곳 저장·라이브)~~ → **2026-06-27 완료**. ~~대량수집 3라운드(61곳)~~/~~대량수집 4라운드(48곳)~~ → **2026-06-29 완료**.
...
- **DB 상태**: `bakery_locations` **484행**(시드 10 + 1R 90 + 2R 85 + 3R 61 + 4R 48 + 5R 190), 그중 **공개 49곳(카테고리 보유)** / **숨김 435곳(`status='draft'`)** / verification_needed 1. 공개 노출 = RLS `status<>'draft' AND published_at IS NOT NULL`(코드 status 필터 없음). 카테고리 채우면 작업대/스크립트로 다시 active 가능. 재실행해도 중복 안 들어감(멱등). ⚠️ approve-and-save는 신규를 active로 저장 → 수집 라운드 뒤엔 `hide-uncategorized.mjs --confirm`로 미분류 숨김 재적용 필요.
- **대량수집 1·2·3·4라운드**: 1R 90곳(작업대 UI 승인) + 2R 85곳 + 3R 61곳 + 4R 48곳(`--approve all --confirm` 자동 승인) → 라이브(DB 294행). `output/stage2-verified.json`은 직전 라운드 결과(다음 라운드 stage2가 덮어씀 — 필요 시 백업). 현재 1~4R 모두 DB 저장 완료.
